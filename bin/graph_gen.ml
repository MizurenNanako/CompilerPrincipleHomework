module CSTGraphGen = struct
  open Dotutils.DotGraph
  module S = Syntactical.CST
  ...
  let of_str s =
    { g_label = Printf.sprintf "\\\"%s\\\"" s; g_arrows = [] }

  let rec of_expr (e : S.expr) =
    match e with
    | UopExpr (u, e') ->
        {
          g_label = "UnaryOpExpr";
          g_arrows = [ of_uop u; of_expr e' ];
        }
    | BopExpr (b, e', e'') ->
        {
    ...
    | CallExpr (e', el) ->
        {
          g_label = "CallExpr";
          g_arrows = [ of_expr e'; of_expr_list el ];
        }
    | AccessExpr (e', e'') ->
        {
          g_label = "AccessExpr";
          g_arrows = [ of_expr e'; of_expr e'' ];
        }
    | MemExpr (e', s) ->
    ...
    | IntAtom i ->
        {
          g_label = "IntAtom";
          g_arrows = [ of_str (Int64.to_string i) ];
        }
    | FloatAtom f ->
        {
          g_label = "FloatAtom";
          g_arrows = [ of_str (string_of_float f) ];
        }

  and of_expr_list el =
    {
      g_label = "ExprList";
      g_arrows = List.map (fun e -> of_expr e) el;
    }

  let rec of_stmt (s : S.stmt) =
    match s with
  ...
  and of_cmpst l =
    ...
    {
      g_label = "CompStmt";
      g_arrows = [ of_def_list c1; of_stmt_list c2 ];
    }

  and of_stmt_list s =
    {
      g_label = "StmtList";
      g_arrows = List.map (fun e -> of_stmt e) s;
    }
  ...
  and of_def d =
    let a, b = d in
    {
      ...
      g_arrows =
        [
          { g_label = "DefSpec"; g_arrows = [ of_spec a ] };
          of_dec_list b;
        ];
    }
    ...
    | Spec CInt ->
        {
          g_label = "Spec";
          g_arrows =
            [ { g_label = "Type"; g_arrows = [ of_str "int" ] } ];
        }
    | Spec CFloat ->
        {
          g_label = "Spec";
          g_arrows =
            [ { g_label = "Type"; g_arrows = [ of_str "float" ] } ];
        }
    | StructSpec s ->
        { g_label = "Struct"; g_arrows = [ of_struct_spec s ] }

  and of_struct_spec s =
    match s with
    | StructDec s ->
        { g_label = "StructDec"; g_arrows = [ of_str s ] }
    | StructDef (Some s, dl) ->
        {
          g_label = "StructDef";
          g_arrows = [ of_str s; of_def_list dl ];
        }
    | StructDef (None, dl) ->
  ...
  and of_dec d =
    match d with
    | vd, Some e ->
        { g_label = "Dec"; g_arrows = [ of_var_dec vd; of_expr e ] }
  ...
  and of_var_dec_list l =
    {
      g_label = "VarDecList";
      g_arrows = List.map (fun e -> of_var_dec e) l;
    }
  ...
  let of_fun_dec fd =
    ...
    in
    {
      g_label = "FunDec";
      g_arrows =
        [ of_str s; { g_label = "FunDecList"; g_arrows = l' } ];
    }

  let of_ext_def (ed : S.ext_def) =
    match ed with
    | ExtVarDec (sp, vdl) ->
        {
          g_label = "ExtVarDec";
  ...
  let of_ext_def_list edl =
    {
      g_label = "ExtDefList";
      g_arrows = List.map (fun e -> of_ext_def e) edl;
    }

  let of_program (pg : S.program) =
