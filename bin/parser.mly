...
%token <int64 * Lexical.Range.t> INT
%token <float * Lexical.Range.t> FLOAT
%token <string * Lexical.Range.t> ID
%token SEMI ";"
%token COMMA ","
%token ASSIGNOP "="
...
var_dec:
| i = ID { VarDecId (fst i) }
| vd = var_dec; LB; n = INT; RB; { VarDecArr (vd,fst n) }

fun_dec:
| s = ID; LP; vl = var_list; RP; { (fst s, vl) }
| s = ID; LP; RP; { (fst s, []) }
...
| e1 = exp; DOT; s = ID; { MemExpr (e1, fst s, snd s) }
| s = ID { IdAtom (fst s, snd s) }
| i = INT { IntAtom (fst i, snd i) }
| f = FLOAT { FloatAtom (fst f, snd f) }
